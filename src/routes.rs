...
use crate::{
    auth::encode_jwt,
    db::{delete_session, new_session, session_from_claims, validate_password},
...
pub(crate) async fn private(
    State(st): State<AppState>,
    claims: Claims,
) -> Result<String, AppError> {
    let conn = st.conn().await?;
    let sess = session_from_claims(conn, claims).await?;
    tracing::debug!("private");
    Ok(format!("Hello {:?}", sess))
}
